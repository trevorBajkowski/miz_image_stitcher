...
def extractandmatch(image1, image2, feature_method, matching_method, first=True, edged=False, scale=0.5, trim=0, augmentations=None):
    ...
    s, h, pp = stitch(im1_, im2_, k1, k2, matches, mask_flag=False, p=p, shp=(stitched.shape), homography=homog)
...
final_mosiac("sample_scene_4", feat_method='brisk', match_method='flann', out_dir="sample_scene_4_out", edged=False, scale = 0.4, augmentations=['rgb-balance', 'smooth'])